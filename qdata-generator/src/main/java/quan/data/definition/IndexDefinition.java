...
public class IndexDefinition extends Definition implements Cloneable {
    ...
    private static Set<String> LEGAL_INDEX_TYPES = CollectionUtils.asSet("normal", "n", "unique", "u", "text", "t");
    ...
    public static boolean isIndex(String index) {
        return isUnique(index) || isNormal(index)|| isText(index);
    }
    ...
    public static void validate(List<IndexDefinition> indexes, List<IndexDefinition> selfIndexes, List<FieldDefinition> fields) {
        ...
        for (FieldDefinition field : fields) {
            ...
            if (!isIndex(field.getIndex())) {
                field.getOwner().addValidatedError(field.getOwner().getValidatedName("的") + field.getValidatedName() + "索引类型[" + field.getIndex() + "]不合法,允许类型" + LEGAL_INDEX_TYPES);
                continue;
            }
    ...
    private void validate() {
        ...
        if (indexType == null) {
            ...
            if (!LEGAL_INDEX_TYPES.contains(indexType)) {
                ownerDefinition.addValidatedError(ownerDefinition.getValidatedName("的") + getValidatedName() + "类型[" + indexType + "]非法,允许类型" + LEGAL_INDEX_TYPES);
            }
        }
